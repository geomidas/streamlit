...
# Logged in --------------------------------------------------------------------------------------
    ...
    with st.expander("💸 Monthly inflows and outflows", expanded=False):
        ...
        with tab1:
            ...
            if len(st.session_state.income_data) > 0:
        ...
        with tab2:
            ...
            if len(st.session_state.spend_data) > 0:
                st.write("##### Monthly Average")
    ...
    with st.expander("💰 Assets", expanded=False):
        ...
        with tab1:
            ...
            if st.toggle("Show Net Worth Data"):
    ...
    with st.expander("🏝️ Financial Freedom / Retirement", expanded=False):
        def curr_fmt(val):
            return locale.currency(val, symbol=False, grouping=True)
        # Load variables
        curr_symbol = st.session_state["curr_symbol"]
